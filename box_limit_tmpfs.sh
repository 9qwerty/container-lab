...
WORKSPACE="$HOME/chroot/limit-tmpfs"
...
# --------------------------------------------------
# Disk quota (tmpfs, RAM-backed, ephemeral)
# --------------------------------------------------
DISK_SIZE="2G"
...
setup_disk() {
    ...
    if mountpoint -q "$CHROOT_DIR"; then
        echo "tmpfs already mounted at $CHROOT_DIR, skipping."
        sudo chown "$(id -u):$(id -g)" "$CHROOT_DIR"
        return
    fi

    sudo mount -t tmpfs -o size="$DISK_SIZE" tmpfs "$CHROOT_DIR"
    sudo chown "$(id -u):$(id -g)" "$CHROOT_DIR"
    echo "Mounted tmpfs -> $CHROOT_DIR (limit: $DISK_SIZE, RAM-backed)"
}
setup_disk
...
setup_cgroup() {
    ...
    # RAM: hard limit, ถ้าเกิน OOM-kill เฉพาะ cgroup นี้
    echo "1G" | sudo tee "$CGROUP_DIR/memory.max" >/dev/null
